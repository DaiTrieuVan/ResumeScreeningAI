...
import { ChevronLeft, ChevronRight, Columns3, Eye, Filter, Search, ShieldCheck } from 'lucide-react';
...
export default function CandidateGrid({ jobId, criteriaSetId, refreshToken, onSelectCandidate }) {
  ...
  return <div className="glass-panel candidate-grid">
    ...
    {data.privacy_mode === 'BLIND' && <div className="privacy-banner" role="status"><ShieldCheck size={16} /><span><strong>Đánh giá ẩn danh đang bật.</strong> Danh tính, liên hệ, nơi làm việc, trường học và tên tệp được che từ máy chủ.</span></div>}
    ...
    <BulkActionBar jobId={jobId} selected={selectedIds} versions={versions} onClear={() => setSelected(new Map())} onCompare={() => setCompareOpen(true)} onCompleted={() => { setSelected(new Map()); setPage(1); setReloadKey((value) => value + 1); }} />
    {loading ? <div className="workspace-state">Đang tải danh sách…</div> : error ? <div className="workspace-state workspace-state--error">{error}</div> : <div className="candidate-grid__table"><table><thead><tr><th><input type="checkbox" aria-label="Chọn trang hiện tại" checked={data.items.length > 0 && data.items.every((item) => selected.has(item.application_id))} onChange={() => setSelected((current) => { const next = new Map(current); const all = data.items.every((item) => next.has(item.application_id)); data.items.forEach((item) => all ? next.delete(item.application_id) : next.set(item.application_id, item.version)); return next; })} /></th>{has('candidate') && <th>Ứng viên</th>}{has('score') && <th>Điểm</th>}{has('gate') && <th>Tiêu chí bắt buộc</th>}{has('stage') && <th>Giai đoạn</th>}<th><span className="sr-only">Thao tác</span></th></tr></thead><tbody>{data.items.map((item) => <tr key={item.application_id}><td><input type="checkbox" aria-label={`Chọn ${item.candidate_name}`} checked={selected.has(item.application_id)} onChange={() => toggle(item)} /></td>{has('candidate') && <td><strong>{item.candidate_name}</strong><span>{item.candidate_email || item.file_name}</span></td>}{has('score') && <td><span className="score-pill score-high">{item.score == null ? '—' : `${item.score}%`}</span></td>}{has('gate') && <td><span className={`gate-pill is-${item.mandatory_gate.toLowerCase()}`}>{item.mandatory_gate}</span></td>}{has('stage') && <td>{STAGES[item.stage] || item.stage}</td>}<td><button aria-label={`Xem hồ sơ ${item.candidate_name}`} onClick={() => onSelectCandidate(item)}><Eye size={14} aria-hidden="true" /> Xem</button></td></tr>)}</tbody></table></div>}
    ...
    {compareOpen && <CandidateComparison jobId={jobId} applicationIds={selectedIds} criteriaSetId={criteriaSetId} onClose={() => setCompareOpen(false)} />}
  </div>;
}
